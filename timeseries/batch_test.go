...
func TestClampBatchSize(t *testing.T) {
	...
	for _, tc := range cases {
		if got := clampBatchSize(tc.in, maxSize); got != tc.want {
			t.Errorf("clampBatchSize(%d, %d) = %d, want %d", tc.in, maxSize, got, tc.want)
		}
	}
}
...
func TestBatchBounds(t *testing.T) {
	...
	total := maxSize*2 + 37
	bounds := batchBounds(total, 0, maxSize)
	if len(bounds) != 3 {
	...
	for i, b := range bounds {
		...
		if size := b[1] - b[0]; size > maxSize {
			t.Errorf("batch %d has %d rows, exceeds max %d", i, size, maxSize)
		}
		prevEnd = b[1]
	}
	if prevEnd != total {
	...
	if b := batchBounds(10, 4, maxSize); len(b) != 3 || b[0] != [2]int{0, 4} || b[2] != [2]int{8, 10} {
