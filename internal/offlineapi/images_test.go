...
func TestServeOfflineImageETag(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(filepath.FromSlash(dir), "singles", "grid", "front", "a", "b"), 0755)
	path := filepath.Join(filepath.FromSlash(dir), "singles", "grid", "front", "a", "b", id+".webp")
	os.WriteFile(path, []byte("webpdata"), 0644)

	get := func(inm string) *httptest.ResponseRecorder {
		w := httptest.NewRecorder()
		r := httptest.NewRequest("GET", "/api/offline/images/"+id+".webp", nil)
		if inm != "" {
			r.Header.Set("If-None-Match", inm)
		}
		s.serveImage(w, r, id+".webp")
		return w
	}
