...
type BanPrice struct {
	...
	Conditions *BanConditions `json:"conditions,omitempty"`
	Quantities *BanQuantities `json:"quantities,omitempty"`
}
...
// FullGradeTags on Add, and the finish suffixes are fixed by processEntry.
// Ordered by grade, best first, then base/foil/etched within a grade.
...
// BanConditions holds per-grade prices as flat fields instead of a map: the
...
// construction (processEntry skips them), so omitempty preserves the wire
// format; keys serialize in conditionTags order.
type BanConditions struct {
...
func (c *BanConditions) ref(tag string) *float64 {
	switch tag {
	case "NM":
		return &c.NM
	case "NM_foil":
...
func (c *BanConditions) set(tag string, price float64) {
...
func (c *BanConditions) get(tag string) float64 {
	if c == nil {
		return 0
	}
	if p := c.ref(tag); p != nil {
...
// BanQuantities is the quantity counterpart of BanConditions.
type BanQuantities struct {
...
func (q *BanQuantities) ref(tag string) *int {
	switch tag {
	case "NM":
		return &q.NM
	case "NM_foil":
...
func (q *BanQuantities) set(tag string, qty int) {
...
func (q *BanQuantities) get(tag string) int {
	if q == nil {
		return 0
	}
	if p := q.ref(tag); p != nil {
